// 1. card_itme 각 요소에 마우스를 오버하면, 각 item들이 위쪽으로 올라오도록 하고 싶다.
// 1_1. 웹브라우저가 > card_tem들이 어떤 요소들인지 알게끔 도와줘야한다.
// 1_1_1. 웹 브라우저는 DOM 객체 모델을 갖고 있으니까 해당 DOM에서 card_item들의 Node를 찾아와야겠다.
...
// 2. 만약 마우스가 A item 위에 있다가, B item으로 이동을 하게 된다면, A item은 원래의 위치로 돌아오게 하고 싶고, B item은 위쪽으로 올라오게 하고 싶다.
...
// 2_1. active 가상클래스가 실행된다. (*text가 검정색 & 두꺼워진다.)
// 2_2. 하단에 lnb요소의 opactity value가 1이 되어야 한다.
// 3. mouseleadv || mouseout 기능을 활용해서 마우스가 옆에 있는 li태그로 이동을 하면 기존이벤트는 제거 // 신규이벤트가 이전에 설정했던 기능을 그대로 구현
...
const bgImg = document.querySelector("#backgroud_img");
