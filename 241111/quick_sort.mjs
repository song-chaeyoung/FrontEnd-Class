...
// 배열의 0번째 인덱스 : 5 (left)
// 배열의 8번째 인덱스 : 8 (rights)

// pivot : 기준값 => 배열 0번째 인덱스
// 비교값 : pivot값을 제외한 나머지 배열 내 아이템
// 비교값 내 첫번째 인덱스 : leftStartIndex = 3
// 비교값 내 마지막 인덱스 : rightStartIndet = 8

// 규칙
// leftStartIndex가 오른쪽으로 한칸씩 이동, pivot값보다 큰 숫자를 만나면 멈춤
// rightStartIndex 왼쪽으로 한칸씩 이동, pivot값보다 작은 숫자를 만나면 멈춤
// 위에 두 인덱스값이 조건에 충족되서 이동을 멈추게 되면, 두 숫자를 swap
// leftStartIndex와 rightStartIndex가 서로 교차가 되면 시점이 오면, rightStartIndex와 pivot의 값을 swap

let arr = [5, 3, 7, 2, 4, 6, 9, 1, 8];
